...
The 32-lesson syllabus for `go-lab`. Each folder is a self-contained mini-lesson with its own `README.md`, runnable code, and (where the concept warrants it) tests. Work through them in the recommended order — see the [syllabus table in the root README](../README.md#syllabus) for the full sequence.

## How this directory is organized

Every top-level folder here matches the pattern `NN-name/`, where `NN` is a two-digit number and `name` is a short kebab-case description. **Folder numbers reflect the order lessons were *authored*, not the order they should be *read*.** When the fundamentals track was added later, its folders got high numbers (19–32) to avoid renumbering older lessons and breaking cross-references. The root README's syllabus table shows the pedagogical order.
...
- Sub-folders when a lesson has multiple worked examples (e.g. `10-panic-and-recover/goroutine-panic/`, `10-panic-and-recover/http-panic/`).
...
go test ./lessons/03-testing-basics

# Run everything in a lesson including sub-packages
go test ./lessons/06-interfaces-and-mocking/...
```

For lessons with a subcommand (like the wcish demo in lesson 32):

```bash
go run ./lessons/32-file-io-and-cli/cmd/wcish -lines README.md
```

The root [`Makefile`](../Makefile) wraps the whole-repo variants — `make test`, `make build`, `make lint`. Prefer those for the "run everything" case; they encode the correct filters (like skipping the intentional-panic packages under `10-panic-and-recover/*/before/`).

## Lessons that intentionally fail

Two sub-lessons under `10-panic-and-recover/` demonstrate what happens **without** panic recovery. Their tests are supposed to fail — that's the pedagogical point.

- `lessons/10-panic-and-recover/goroutine-panic/before/`
- `lessons/10-panic-and-recover/http-panic/before/`

The matching `after/` packages show the fix and pass. `make test` filters the `before/` packages out so a clean checkout produces green output; use `make test-all` if you want to see the intentional failures. See [`lessons/10-panic-and-recover/README.md`](./10-panic-and-recover/README.md) for the whole picture.

## Lessons that live in their own Go module

Two lessons have their own `go.mod` (visible with `find lessons -name go.mod`):

- **`lessons/13-design-patterns/`** — a separate module for historical reasons; contains no external dependencies. Would fold cleanly back into the root module in a future refactor.
- **`lessons/14-production-api/`** — a separate module because it depends on packages (Cobra, Viper, Zap, Swagger) that shouldn't leak into every simple lesson.

`go test ./...` at the repo root only exercises the root module. The CI workflow builds, lints, and vuln-scans the submodules independently (`Build submodules` and `Lint (lessons/14-production-api)` jobs). If you want to work inside one:

```bash
cd lessons/14-production-api
go test ./...
```
...
| New to Go, know another language | Lesson [01-hello](./01-hello/), then follow the syllabus table in order — the fundamentals track (19–24, 26–32) is threaded in at the right dependency points. |
| Experienced Go dev, browsing | Any lesson that looks interesting. Concurrency: 07–09. HTTP: 11, 16–18. Testing shape: 06 and 30-json. |
