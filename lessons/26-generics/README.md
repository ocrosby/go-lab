...
> **Recommended after lesson 07 (interfaces).** Constraints are interface types under the hood; understanding interfaces first makes generics land more cleanly.
...
- Lesson 02: functions.
- Lesson 07: interfaces (constraints are a kind of interface).
- Lesson 22: slices and maps (generic functions on them are where you'll use this most).

## Run it

```bash
go test ./lessons/26-generics
```
...
- **Generic types with pointer receivers.** Same rules as non-generic pointer receivers (lesson 23) — but the mental picture takes a beat longer. Read carefully.
...
- **If you started with the fundamentals track**, cycle back to [06-composition](../06-composition/) and continue through 06, 07, ... up to 18.
- **If you're ready for a project**, jump to [15-production-api](../15-production-api/) and build.
