...
> **Recommended before lessons 07 (goroutines) and 11 (HTTP)** — both of which use `if err != nil` heavily without ever explaining it.
...
- Lesson 19: types (the `error` interface).
- Lesson 22: pointers (custom error types often use pointer receivers).

## Run it

```bash
go test ./lessons/23-error-handling
```
...
- **Next lesson (recommended):** [24-defer-and-cleanup](../24-defer-and-cleanup/) — the `defer` keyword you'll want for reliable resource cleanup in error paths.
