...
- Lesson 19: types.
- Lesson 23: error handling (every I/O call returns an error).
- Lesson 24: `defer` for closing files.

## Run it

```bash
go test -race ./lessons/32-file-io-and-cli
```
...
```bash
go run ./lessons/32-file-io-and-cli/cmd/wcish -lines /etc/hosts
go run ./lessons/32-file-io-and-cli/cmd/wcish -words -bytes README.md
```
...
Notice `defer f.Close()` right after the error check (lesson 24). `bufio.Scanner` splits on newlines by default; call `scanner.Split(bufio.ScanWords)` for word-splitting, `ScanBytes` for byte-by-byte, `ScanRunes` for Unicode code points.
...
4. Write a function that reads a file and returns the top-10 most-frequent words. Use `map[string]int` (lesson 21) and `slices.SortFunc` (lesson 25).
...
- **Cycle back to lesson 05** and continue the main syllabus if you started on the fundamentals track.
- **Try building a small tool of your own** in a fresh module (`go mod init github.com/you/name`) using everything from lessons 19-32. Log tailer, config validator, whatever you'd find useful.
