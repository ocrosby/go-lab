...
> **Recommended before lesson 12 (HTTP) and lesson 15 (production API).** Both use `context.Context` on every method without ever introducing it.
...
- Lesson 08: goroutines and channels — `ctx.Done()` returns a channel.
- Lesson 25: `defer` — every `WithCancel`/`WithTimeout` returns a cancel func you should `defer cancel()`.
- Lesson 28: functions and closures (goroutines started with `go func()` capture ctx).

## Run it

```bash
go test -race ./lessons/30-context
```
...
- **Next lesson:** [31-json-and-struct-tags](../31-json-and-struct-tags/) — how struct tags direct `encoding/json`, and the safe body-decoding pattern lesson 17 already uses.
