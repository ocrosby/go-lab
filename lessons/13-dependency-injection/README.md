...
This is the same idea lesson 07 introduced with interfaces — this lesson names it and shows it applied to a small, whole program.

## Prerequisites

- Lesson 06: composition (structs holding other types).
- Lesson 07: interfaces satisfied structurally.

## Run it

```bash
go run ./lessons/13-dependency-injection
```
...
Java has Spring, Python has FastAPI's `Depends`, .NET has built-in DI containers. Go has… `func NewX(deps ...) *X`. Because Go's interfaces are satisfied structurally (lesson 07), a manual constructor is enough for the vast majority of cases. There *are* Go DI libraries (`wire`, `uber-fx`, `google/wire`) for very large graphs, but reach for them only when hand-wiring becomes painful — usually deep into a production codebase.

Lesson 15 (`15-production-api`) shows hand-wired DI on a real service.
...
- **A "fat" interface that most implementations don't fully implement.** If your interface has 12 methods and each mock has to implement all 12 (even the ones the test doesn't care about), you probably need to split the interface. See lesson 07.
...
- **Next lesson:** [14-design-patterns](../14-design-patterns/) — classic OO design patterns rendered in idiomatic Go.
