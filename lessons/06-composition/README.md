...
```bash
go build ./lessons/06-composition
```
...
Both styles are valid Go. Embedding is more concise; explicit fields are more, well, explicit. The rest of this repo prefers small, purpose-built types over deep embedding trees — see [lesson 07](../07-interfaces-and-mocking/) for the alternative.
...
Also notice you never *declare* that `Ninja` implements `NinjaInterface`. Go checks structurally at compile time — if the methods match, the interface is satisfied. This is called **structural typing** or **duck typing at compile time**. Lesson 07 leans on this heavily.
...
- **Next lesson:** [07-interfaces-and-mocking](../07-interfaces-and-mocking/) — how to design small interfaces at the point of use, and how to generate mocks for tests.
