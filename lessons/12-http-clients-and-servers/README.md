...
`net/http` is Go's flagship standard-library package. A working REST API touches maybe a dozen of its types and interfaces — this lesson introduces the smallest useful ones, and lessons 17–18 go into depth on routing, middleware, and production client patterns.

## Prerequisites

- Lesson 07: interfaces (mocking an HTTP client requires them).
...
```bash
go test ./lessons/12-http-clients-and-servers/...
```
...
| [`jsonplaceholder/`](./jsonplaceholder/) | A bigger, structured HTTP client that hits a real public API, with a service layer and a mockable transport. Ties together lessons 07 and 11. |

## Where to go next

The three add-on lessons that turn this foundation into a REST-ready toolkit:

- **[17-restful-routing](../17-restful-routing/)** — REST verbs and path parameters using the Go 1.22+ ServeMux.
- **[18-http-middleware](../18-http-middleware/)** — the middleware chain every server needs.
- **[19-http-client-depth](../19-http-client-depth/)** — production HTTP client: custom transport, timeouts, retries, testing.

## Common pitfalls

- **Forgetting `resp.Body.Close()`** — leaks a connection every call. Always `defer resp.Body.Close()` right after checking the error.
- **Using the default `http.Client`** for production requests — no timeout, follows up to 10 redirects. Fine for scripts; not fine for services. See lesson 19.
