...
```bash
go run ./lessons/20-variables-and-types
```
...
- `[]T` — slice of T (lesson 22).
- `map[K]V` — map of K to V (lesson 22).
- `*T` — pointer to T (lesson 23).
- `struct { ... }` — record (lesson 12).
...
- **Next lesson:** [21-control-flow](../21-control-flow/) — Go's `if`, `switch`, and `for` (the only loop construct in the language).
