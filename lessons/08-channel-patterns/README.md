...
Lesson 07 taught the three primitives (goroutines, channels, `select`). Real code composes them into repeatable shapes. Once you can name a shape ("this is a fan-in") you can spot it in someone else's code, and you can design your own concurrent code by picking a shape rather than inventing one from scratch.

## Prerequisites

- Lesson 07: goroutines, channels, `select`, closing channels.
...
```bash
go test ./lessons/08-channel-patterns/channels/...
```
...
Lesson 09 (`09-worker-pools`) is a full fan-out example. `docs/csp-and-go-concurrency.md` diagrams both.
...
- **Next lesson:** [09-worker-pools](../09-worker-pools/) — a fan-out pattern applied end-to-end, with bounded concurrency.
