...
> **Recommended before lessons 10 (panic/recover) and 11 (HTTP).** Both use `defer` prominently — closing HTTP response bodies, unlocking mutexes, recovering from panics — without ever explaining the timing rules.
...
- Lesson 20: control flow (`for`, `return`).
- Lesson 23: error handling (why you need cleanup in error paths).

## Run it

```bash
go test ./lessons/24-defer-and-cleanup
```
...
`recover` only works inside a deferred function. This is the pattern lesson 10 (panic-and-recover) covers in depth.
...
4. Combine `defer` with `recover` to turn a panic into an error return. Compare with lesson 10's HTTP-handler and worker-pool examples.
...
- **Next lesson (recommended):** [05-composition](../05-composition/) — you now have every fundamental you need to start the main syllabus properly.
