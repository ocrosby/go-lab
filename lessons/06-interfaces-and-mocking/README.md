...
```bash
go test -race ./lessons/06-interfaces-and-mocking/...
```
...
For lesson 06 as it stands now: **yes**. Extract methods in `AccordFactory`, rename fields in `AccordBuilder`, inline the ignition state machine — the tests survive because none of them name the shape of the code under test.
