...
> **Recommended before lesson 06 (composition)** and every lesson after. Slices and maps appear on nearly every page from lesson 06 onward without ever being introduced formally.
...
- Lesson 21: control flow (`for range`).

## Run it

```bash
go run ./lessons/22-slices-and-maps
```

And run the tests:

```bash
go test ./lessons/22-slices-and-maps
```

Expected output (last line):

```text
ok  	github.com/ocrosby/go-lab/lessons/22-slices-and-maps	...
```
...
- **`len(m)` is O(1), fine to call in loops.** `len(s)` for a slice is also O(1). But `len(str)` on a string returns *bytes*, not characters (see lesson 20).
...
- **Next lesson (recommended):** [23-pointers](../23-pointers/) — the last piece of Go you need before methods and interfaces start clicking.
