...
- Lesson 12: `http.Handler`, `http.HandlerFunc`.
- Lesson 17: full REST handlers to wrap.
- Lesson 11: panic recovery basics (this lesson generalizes what you saw there).

## Run it

```bash
go test -race ./lessons/18-http-middleware
```
...
- **Next lesson:** [19-http-client-depth](../19-http-client-depth/) — the client-side equivalents of everything you just learned: `http.RoundTripper` is the client's middleware seam, custom `Transport` is the connection-pool tuning knob, `context.WithTimeout` is the per-request cancellation primitive.
