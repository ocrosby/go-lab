...
> **Recommended after lesson 07 (interfaces).** Interface values are the input; type assertions are how you get back to concrete types.
...
- Lesson 07: interfaces (satisfy them structurally).
- Lesson 24: error handling (already saw `errors.As` — this generalizes the pattern).

## Run it

```bash
go test -race ./lessons/32-type-assertions
```
...
From lesson 24. `errors.As(err, &target)` walks the error's wrap chain looking for something the target's type. It's a type assertion that follows wraps:
...
You'll see this at the top of adapter files throughout this repo — e.g. `lessons/07-interfaces-and-mocking` uses it in the test files.
...
4. Add a compile-time check `var _ error = (*ValidationError)(nil)` from lesson 24's ValidationError to its file. Verify it catches missing-method bugs (remove `Error() string` and watch the check fail).
...
- **Next lesson:** [33-file-io-and-cli](../33-file-io-and-cli/) — `os.Args`, `flag`, `os.Open`, `io.Reader`, `bufio.Scanner` — the last set of standard-library basics you need to build a real command-line tool.
