...
```bash
go test ./lessons/11-http-clients-and-servers/server-basics
```

Expected output:

```text
ok  	github.com/ocrosby/go-lab/lessons/11-http-clients-and-servers/server-basics	...
```
...
import "github.com/ocrosby/go-lab/lessons/11-http-clients-and-servers/server-basics"
...
3. **`http.ServeMux`** — the request router. Match URLs to handlers. Go 1.22 added method-in-pattern support; see lesson 16.
...
3. Add a `/panic` handler that calls `panic("test")`. Watch the whole process crash when you `curl` it. Then look at lesson 17 — that's what middleware is for.
...
- **REST-specific routing:** [`../../16-restful-routing/`](../../16-restful-routing/).
- **Middleware:** [`../../17-http-middleware/`](../../17-http-middleware/).
