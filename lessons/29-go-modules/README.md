...
This repo uses submodules (`lessons/14-design-patterns/go.mod`, `lessons/15-production-api/go.mod`) — those are separate modules within the same repo, not a workspace, but the same principles apply.
...
- **Next lesson:** [30-context](../30-context/) — `context.Context` for cancellation, deadlines, and request-scoped values. The last built-in-package concept you need before the concurrency and HTTP lessons make full sense.
