...
This lesson replaces the client-side content of lessons 12 and 14 with production-ready shapes.

## Prerequisites

- Lesson 12: basic `http.Client` and `http.Get`.
- Lesson 08: goroutines and channels (for understanding what `context.Context` propagates through).
- Lesson 13: dependency injection (the `RoundTripper` pattern is DI applied to the HTTP client).

## Run it

```bash
go test -race ./lessons/19-http-client-depth
```
...
Lesson 12's `jsonplaceholder/` defines a custom `IHttpClient` interface so a mock can be swapped in. That works, but it's noise — Go already has a perfect testing seam:
