...
- Lesson 06: composition.
- Lesson 07: interfaces satisfied structurally.
- Lesson 13: dependency injection (the same idea shows up here in constructor form).
...
```bash
go run ./lessons/14-design-patterns/creational/builder
go run ./lessons/14-design-patterns/creational/prototype
go run ./lessons/14-design-patterns/creational/singleton   # if present
go run ./lessons/14-design-patterns/structural/adpater     # note: folder is "adpater" (typo)
```
...
"Only one instance ever exists." In Go, use `sync.Once` for thread-safe lazy initialization, or a package-level variable initialised in `init()`. The pattern is often overused; before reaching for it, ask "would injecting the dependency work instead?" (See lesson 13.)
...
3. Sketch a Decorator pattern in Go on paper. Given lesson 07's material, how much code do you need? (Answer: less than you'd think.)
...
- **Next lesson:** [15-production-api](../15-production-api/) — the patterns used together in a real service with hexagonal architecture.
