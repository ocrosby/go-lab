...
> **Recommended before lesson 12 (HTTP) and lesson 15 (production API).** Both `json.NewDecoder(r.Body).Decode(&v)` and `json:"email"` struct tags appear in nearly every handler without any prior explanation.

## Why it matters

Almost every Go program that talks to another system uses JSON. The `encoding/json` package handles the encoding, but it needs guidance for anything non-trivial: renaming fields, omitting empty values, ignoring fields entirely, handling optional fields, catching typos. That guidance lives in struct tags — the string after the field type — and the safe decoding pattern lesson 17 uses.

## Prerequisites

- Lesson 20: types.
- Lesson 27: structs and methods.
- Lesson 24: error handling.

## Run it

```bash
go test -race ./lessons/31-json-and-struct-tags
```
...
Lesson 17 uses this exact pattern in `decodeJSON`. Use it in every request handler.
...
- **Next lesson:** [32-type-assertions](../32-type-assertions/) — the `x.(T)` and `switch v := x.(type)` forms for extracting concrete types out of interface values.
